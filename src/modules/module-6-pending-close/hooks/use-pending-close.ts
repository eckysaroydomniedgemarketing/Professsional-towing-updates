...
export function usePendingClose() {
  ...
  useEffect(() => {
    ...
    if (isPolling) {
      interval = setInterval(async () => {
        try {
          const response = await fetch('/api/module-6/status');
          if (response.ok) {
          ...
          console.error('[PENDING-CLOSE] Error polling status:', error);
        }
      }, 1000);
    }
  ...
  const handleStart = useCallback(async () => {
    try {
      const response = await fetch('/api/module-6/start-workflow', {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        console.error('[PENDING-CLOSE] Failed to start workflow:', error);
      ...
      console.error('[PENDING-CLOSE] Error starting workflow:', error);
  ...
  const handleStop = useCallback(async () => {
    try {
      const response = await fetch('/api/module-6/stop', {
        method: 'POST'
      });
      ...
      console.error('[PENDING-CLOSE] Error stopping workflow:', error);
    }
  }, []);
  
  const handleNextCase = useCallback(async () => {
    try {
      const response = await fetch('/api/module-6/continue', {
        method: 'POST'
      });
      ...
      console.error('[PENDING-CLOSE] Error continuing workflow:', error);
    }
  }, []);
  ...
  const exportToCsv = useCallback(() => {
    exportUtil(processedCases, 'pending-close-report');
  }, [processedCases]);
