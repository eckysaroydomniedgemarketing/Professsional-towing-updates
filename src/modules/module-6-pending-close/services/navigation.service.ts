...
export class NavigationService {
  ...
  async navigateToPendingClose(): Promise<boolean> {
    if (!this.page) {
      console.error('[PENDING-CLOSE] Page not initialized');
      return false;
    }
    
    try {
      const url = `${this.baseUrl}/v2/main/view_cases.php?status=Pending+Close`;
      ...
      console.log('[PENDING-CLOSE] Successfully navigated to Pending Close page');
      return true;
    } catch (error) {
      console.error('[PENDING-CLOSE] Navigation error:', error);
      return false;
    }
  }
  ...
  async openCaseDetail(caseId: string): Promise<boolean> {
    ...
    try {
      ...
      if (newPage) {
        ...
        console.log(`[PENDING-CLOSE] Opened case ${caseId} in new tab`);
        return true;
      }
      ...
      console.error('[PENDING-CLOSE] Error opening case:', error);
      return false;
    }
  }
  ...
  async extractCaseList(): Promise<string[]> {
    ...
    try {
      ...
      console.log(`[PENDING-CLOSE] Found ${caseIds.length} cases`);
      return caseIds;
      
    } catch (error) {
      console.error('[PENDING-CLOSE] Error extracting cases:', error);
      return [];
    }
  }
  ...
  async closeCurrentTab(): Promise<void> {
    ...
    try {
      ...
      console.error('[PENDING-CLOSE] Error closing tab:', error);
    }
  }
}