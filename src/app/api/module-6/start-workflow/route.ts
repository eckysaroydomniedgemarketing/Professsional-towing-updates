...
import { PendingCloseWorkflowService } from '@/modules/module-6-pending-close/services/pending-close-workflow.service';
...
export let workflowService: PendingCloseWorkflowService | null = null;
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!workflowService) {
      workflowService = new PendingCloseWorkflowService();
    }
    ...
    if (!page) {
      ...
      console.log('[MODULE-6] Creating RDNPortalService');
      portalService = new RDNPortalService(credentials);
      
      console.log('[MODULE-6] Initializing portal service');
      await portalService.initialize();
      
      // Add small delay to ensure initialization is complete
      await new Promise(resolve => setTimeout(resolve, 300));
      
      console.log('[MODULE-6] Navigating to login page');
      ...
      console.log('[MODULE-6] Getting authenticated page from browser manager');
      const browserManager = (portalService as any).browserManager;
      page = browserManager.getPage();
      
      if (!page) {
        console.error('[MODULE-6] Failed to get page from browser manager', {
      ...
      console.log('[MODULE-6] Successfully got authenticated page');
    }
    ...
    workflowService.startWorkflow(mode).catch(error => {
      console.error('[MODULE-6] Workflow error:', error);
    });
    ...
    return NextResponse.json({
      success: true,
      message: 'Pending Close workflow started',
      mode,
      state: finalState
    });
  } catch (error) {
    console.error('[MODULE-6] Error starting workflow:', error);
    return NextResponse.json(
      { 
